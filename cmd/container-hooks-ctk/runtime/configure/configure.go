...
const (
	...
	// defaultRuntimeExecutable is the default container hooks runtime executable file name
	defaultRuntimeExecutable      = "container-hooks-runtime"
	defaultRuntimeExpecutablePath = "/usr/bin/container-hooks-runtime"
...
func (m command) build() *cli.Command {
	...
	configure := cli.Command{
		Name:  "configure",
		Usage: "Add container hooks runtime to the specified container engine",
	...
	configure.Flags = []cli.Flag{
		&cli.StringFlag{
			Name:        "runtime",
			Usage:       "Target runtime engine [docker, containerd, crio]",
			Value:       defaultRuntime,
			Destination: &config.runtime,
		},
		&cli.StringFlag{
			Name:        "config-file",
			Usage:       "Path to the config file for the target runtime engine\n\tUse default config path of the target runtime engine if not specified",
			Destination: &config.configFilePath,
		},
		&cli.StringFlag{
			Name:        "name",
			Usage:       "Specify the name of the container hooks runtime",
			Value:       defaultRuntimeName,
			Destination: &config.trustedRuntime.name,
		},
		&cli.StringFlag{
			Name:        "path",
			Usage:       "Specify the path to the container hooks runtime",
			Value:       defaultRuntimeExpecutablePath,
			Destination: &config.trustedRuntime.path,
		},
		&cli.BoolFlag{
			Name:        "set-as-default",
			Aliases:     []string{"default"},
			Usage:       "Set the container hooks runtime as the default runtime",
			Destination: &config.trustedRuntime.setAsDefault,
		},
	}
...
func (m command) validateFlags(c *cli.Context, config *config) error {
	...
	if !filepath.IsAbs(config.trustedRuntime.path) {
		return fmt.Errorf("the container hooks runtime path %q is not an absolute path", config.trustedRuntime.path)
	}

	return nil
}

// configureConfigFile updates the specified container engine config to enable the container hooks runtime
func (m command) configureConfigFile(c *cli.Context, config *config) error {
	...
	err = cfg.AddRuntime(
		config.trustedRuntime.name,
		config.trustedRuntime.path,
		config.trustedRuntime.setAsDefault,
	)
	if err != nil {
